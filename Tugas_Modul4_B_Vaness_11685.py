...
def scatter(model, model_name, data, new_point, features, color_scale, title):
    ...
    data[f"{model_name} Cluster"] = clusters
    ...
    fig = px.scatter_3d(data, x='Avg_Credit_Limit', y='Total_Credit_Cards', z='Total_visits_online',
                        color=f"{model_name} Cluster", title=title, color_continuous_scale=color_scale)
...
st.set_page_config(
    page_title="11685 - Unsupervised Learning",  
...
if uploaded_file is not None:
    ...
    st.markdown("<h1 style='text-align: center;'>Unsupervised Learning - 11685</h1>", unsafe_allow_html=True)  # YYYYY diisi dengan nama panggilan
    st.dataframe(input_data)

    
    model_directory = 'C:\KULIAH\Semester 5\Machine Learning\Tugas4_B_11685'
    model_path = {
